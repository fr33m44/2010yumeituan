...
header('Pragma: no-cache');
	...
	$sql = "SELECT group_id,group_name, cat_id, group_image,start_time,end_time,market_price,ext_info,city_id " .
    ...
    while ($group_buy = $GLOBALS['db']->fetchRow($res))
	{
        ...
        $sql = 'SELECT keywords FROM' . $ecs->table('category') . " WHERE cat_id='$group_buy[cat_id]'";
		$class_e['keywords']= $db->getOne($sql);
		$xml .= '<class>'.$class_e['keywords'].'</class>';
