...
elseif ($_REQUEST['act'] == 'info')
{
    ...
    $sql = "SELECT o.* FROM " . $ecs->table('order_goods') . " AS o WHERE o.order_id = '$order[order_id]'";
...
elseif ($_REQUEST['act'] == 'operate')
{
	...
	if ($goods_list)
	{
		...
		elseif ($goods_value['extension_code'] == 'group_buy')
		{
...
elseif ($_REQUEST['act'] == 'operate_post')
{
	...
	if ($order['extension_code'] == 'group_buy' && !$GLOBALS['_CFG']['make_group_card'])
	{
	    $is_send = !$GLOBALS['_CFG']['send_group_sms'];
	    send_group_cards($order_id, $order['order_sn'], $order['user_id'], $order['mobile'], $is_send);
	}
	...
	if ($order['extension_code'] == 'group_buy')
	{
	    ...
	    send_group_cards($order_id, $order['order_sn'], $order['user_id'], $order['mobile']);
	}
	...
	if ($delivery_id)
	{
		...
		foreach ($goods_list as $value)
		{
		    ...
		    if (empty($value['extension_code']) || $value['extension_code'] == 'virtual_card' || $value['extension_code'] == 'group_buy')
		    {
...
elseif ($_REQUEST['act'] == 'get_goods_info')
{
    ...
    $sql = "SELECT o.*, g.group_image AS goods_thumb, g.group_stock AS storage, o.goods_attr " .
	    "FROM " . $ecs->table('order_goods') . " AS o " .
	    "LEFT JOIN " . $ecs->table('group_activity') . " AS g ON o.goods_id = g.group_id " .
	    //"LEFT JOIN " . $ecs->table('brand') . " AS b ON g.brand_id = b.brand_id " .
...
elseif ($_REQUEST['act'] == 'get_excel_order')
{
    admin_priv('download_order');
    $filter = array();
    ...
    $filter['shipping_start_time'] = empty($_REQUEST['shipping_start_time']) ? '' : (strpos($_REQUEST['shipping_start_time'], '-') > 0 ? local_strtotime($_REQUEST['shipping_start_time']) : $_REQUEST['shipping_start_time']);

    $filter['shipping_end_time'] = empty($_REQUEST['shipping_end_time']) ? '' : (strpos($_REQUEST['shipping_end_time'], '-') > 0 ? local_strtotime($_REQUEST['shipping_end_time']) : $_REQUEST['shipping_end_time']);
	...
	if ($filter['shipping_status'] == 1)
	{
	    ...
	    $where .= " AND o.shipping_status in (1,2,4)";
	}
	else
	{
    ...
    if ($filter['shipping_start_time'])
    {

	$where .= " AND o.shipping_time >= '$filter[shipping_start_time]'";
    }

    if ($filter['shipping_end_time'])
    {

	$where .= " AND o.shipping_time <= '$filter[shipping_end_time]'";
    }
    ...
    $where .= " AND o.order_status " . db_create_in(array(OS_CONFIRMED, OS_SPLITED)) .
...
function operable_list($order)
{
	...
	else
	{
		...
		if ($priv_list['ss'])
		{
			...
			$sql = "SELECT is_real FROM " . $GLOBALS['ecs']->table('order_goods') .
				" WHERE order_id='$order[order_id]'";
			$is_real = $GLOBALS['db']->getOne($sql);
			if ($is_real == 1)
			{
			    $list['send_card'] = true;
			}
	...
	if ($order['extension_code'] == 'group_buy')
	{
	    ...
	    $sql = "SELECT succeed_time FROM " .
		    $GLOBALS['ecs']->table('group_activity') . " WHERE group_id='$order[extension_id]'";

	    $succeed_time = $GLOBALS['db']->getOne($sql);

	    if ($succeed_time == '0')
	    {

		unset($list['split']);
		unset($list['send_card']);
...
function order_list()
{
	...
	$where = "WHERE o.extension_code='group_buy'";
	...
	if ($filter['group_buy_id'])
	{
	    ...
	    $where .= " AND o.extension_id = '$filter[group_buy_id]' ";
	}
...
function get_package_goods_list($package_id)
{
    ...
    $sql = "SELECT pg.goods_id, g.goods_name, g.goods_number, p.goods_attr, p.product_id, pg.goods_number AS
...
function update_order_goods($order_id, $_sended, $goods_list = array())
{
	...
	elseif (!is_array($value))
	{
		...
		if ($goods['rec_id'] == $key && $goods['is_real'] == 2)
		{
...
function get_site_root_url()
{
    ...
    return 'http://' . $_SERVER['HTTP_HOST'] . str_replace('/' . GROUPON_PATH . '/order.php', '', PHP_SELF);
}

function get_excel_order($where = '')
{
	...
	$filename = ecs_iconv('UTF8', 'GB2312', $filename);
    }
	...
	//echo ecs_iconv('UTF8', 'GB2312', '发货时间') ."\t";

	echo ecs_iconv('UTF8', 'GB2312', '订单号') . "\t";

	echo ecs_iconv('UTF8', 'GB2312', '购买数量') . "\t";

	echo ecs_iconv('UTF8', 'GB2312', '收件人') . "\t";

	echo ecs_iconv('UTF8', 'GB2312', '收件人手机') . "\t";

	echo ecs_iconv('UTF8', 'GB2312', '收件人地址') . "\t";

	echo ecs_iconv('UTF8', 'GB2312', '邮编') . "\t";

	echo ecs_iconv('UTF8', 'GB2312', '备注') . "\t";



	//echo ecs_iconv('UTF8', 'GB2312', '商品总金额') ."\t";
	//echo ecs_iconv('UTF8', 'GB2312', '配送费用') ."\t";

	echo ecs_iconv('UTF8', 'GB2312', '订单总金额') . "\t";

	echo ecs_iconv('UTF8', 'GB2312', '已付金额') . "\t";

	//echo ecs_iconv('UTF8', 'GB2312', '付款时间') ."\t";
	//echo ecs_iconv('UTF8', 'GB2312', '支付方式') ."\t";

	echo ecs_iconv('UTF8', 'GB2312', '红包金额') . "\t";

	echo ecs_iconv('UTF8', 'GB2312', '使用余额') . "\t";

	//echo ecs_iconv('UTF8', 'GB2312', '折扣金额') ."\t";
	//echo ecs_iconv('UTF8', 'GB2312', '应付金额') ."\t";
	//echo ecs_iconv('UTF8', 'GB2312', '会员名称') ."\t";
	//echo ecs_iconv('UTF8', 'GB2312', '下单时间') ."\t";*/
    ...
    $sql = "SELECT o.order_sn, o.goods_amount, o.shipping_fee, o.bonus, o.money_paid,o.order_amount, o.discount,o.add_time,o.shipping_time,o.pay_time,o.pay_name,o.postscript,o.consignee,o.address,o.mobile,o.zipcode"
	    . ",og.goods_number AS goods_num, IFNULL(u.user_name, '" . $GLOBALS['_LANG']['anonymous'] . "') AS buyer,"
	    . "(" . order_amount_field('o.') . ") AS total_fee,surplus FROM "
	...
	// echo local_date('Y-m-d H:i', $val['shipping_time']) . "\t";

	echo $val['order_sn'] . "\t";

	echo $val['goods_num'] . "\t";

	echo ecs_iconv('UTF8', 'GB2312', $val['consignee']) . "\t";

	echo $val['mobile'] . "\t";

	echo ecs_iconv('UTF8', 'GB2312', $val['address']) . "\t";

	echo $val['zipcode'] . "\t";

	echo ecs_iconv('UTF8', 'GB2312', $val['postscript']) . "\t";



	/* echo $val['goods_amount'] . "\t";

	  echo $val['shipping_fee'] . "\t"; */

	echo $val['total_fee'] . "\t";

	echo $val['money_paid'] . "\t";

	// echo local_date('Y-m-d H:i', $val['pay_time']) . "\t";
	//echo ecs_iconv('UTF8', 'GB2312',  $val['pay_name']) ."\t";
	...
	/* echo $val['order_amount'] . "\t";
	  ...
	  echo ecs_iconv('UTF8', 'GB2312',  $val['buyer']) ."\t";

	  echo local_date('Y-m-d H:i', $val['add_time']). "\t"; */
