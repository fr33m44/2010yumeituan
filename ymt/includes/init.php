...
/* 取得当前ecshop所在的根目录 */
if(!defined('GROUPON_PATH'))
{
    define('GROUPON_PATH','ymt');
}
define('ROOT_PATH', str_replace(GROUPON_PATH . '/includes/init.php', '', str_replace('\\', '/', __FILE__)));
...
require(ROOT_PATH . GROUPON_PATH . '/includes/lib_main.php');
require(ROOT_PATH . GROUPON_PATH . '/includes/cls_exchange.php');
...
require(ROOT_PATH . GROUPON_PATH . '/languages/common.php');
require(ROOT_PATH . GROUPON_PATH . '/languages/log_action.php');

if (file_exists(ROOT_PATH . GROUPON_PATH . '/languages/' . basename(PHP_SELF)))
{
    include(ROOT_PATH . GROUPON_PATH . '/languages/' . basename(PHP_SELF));
}
...
if (!file_exists('../temp/compiled/group'))
{
    @mkdir('../temp/compiled/group', 0777);
    @chmod('../temp/compiled/group', 0777);
}
...
$smarty->template_dir  = ROOT_PATH . GROUPON_PATH . '/templates';
$smarty->compile_dir   = ROOT_PATH . 'temp/compiled/group';
if ((DEBUG_MODE & 2) == 2)
{
...
if ($_REQUEST['act'] != 'login' && $_REQUEST['act'] != 'signin' &&
    $_REQUEST['act'] != 'forget_pwd' && $_REQUEST['act'] != 'reset_pwd' && $_REQUEST['act'] != 'check_order')
{
    $groupon_admin = preg_replace('/:\d+/', '', $ecs->url());
    if (!empty($_SERVER['HTTP_REFERER']) &&
        strpos(preg_replace('/:\d+/', '', $_SERVER['HTTP_REFERER']), $groupon_admin) === false)
    {
        if (!empty($_REQUEST['is_ajax']))
        {
            make_json_error($_LANG['priv_error']);
