...
if ($_REQUEST['act'] == 'list')
{
    ...
    /* 获得当前的模版的信息 */
    $curr_template = $_CFG['formwork'];
    $curr_style = $_CFG['stylename'];
    /* 获得可用的模版 */
    $available_templates = array();
    $template_dir        = @opendir(ROOT_PATH . 'template/');
    while ($file = readdir($template_dir))
    {
        if ($file != '.' && $file != '..' && is_dir(ROOT_PATH. 'template/' . $file) && $file != '.svn' && $file != 'index.htm')
        {
...

{
    ...
    $template_file    = '../template/' . $curr_template . '/' . $_POST['template_file'] . '.dwt';
...

    {
...

        {
...

            {
                ...
                if (!file_exists('../template/' . $curr_template . $lib['library']))
                ...
                $lib_content     = file_get_contents('../template/' . $curr_template . $lib['library']);
...

    {
        ...
        sys_msg(sprintf($_LANG['modify_dwt_failed'], 'template/' . $curr_template. '/' . $_POST['template_file'] . '.dwt'), 1, null, false);
...

{
    ...
    $library_path  = '../template/' . $curr_template . '/library';
...

{
    ...
    $sql = "UPDATE " .$GLOBALS['ecs']->table('shop_config'). " SET value = '$tpl_name' WHERE code = 'formwork'";
...

{
    ...
    $done = $zip->zip('../template/' . $tpl . '/', $filename);
...

{
    ...
    $library = load_library($_CFG['formwork'], trim($_GET['lib']));
...

{
    ...
    $lib_file = '../template/' . $_CFG['formwork'] . '/library/' . $_POST['lib'] . '.lbi';
...

    {
        ...
        @file_put_contents('../temp/backup/library/' . $_CFG['formwork'] . '-' . $_POST['lib'] . '.lbi', $org_html);
...

    {
        ...
        make_json_error(sprintf($_LANG['update_lib_failed'], 'template/' . $_CFG['formwork'] . '/library'));
...

{
    ...
    $lib_file   = '../template/' . $_CFG['formwork'] . '/library/' . $lib_name . '.lbi';

    $lib_file   = str_replace("0xa", '', $lib_file); // 过滤 0xa 非法字符

    $lib_backup = '../temp/backup/library/' . $_CFG['formwork'] . '-' . $lib_name . '.lbi';
...

{
    ...
    $sql = "SELECT DISTINCT(remarks) FROM " . $ecs->table('template') . " WHERE theme = '" . $_CFG['formwork'] . "' AND remarks > ''";
    ...
    $sql = "SELECT DISTINCT(filename) FROM " . $ecs->table('template') . " WHERE theme = '" . $_CFG['formwork'] . "' AND remarks = ''";
...

{
    ...
    $sql = "SELECT COUNT(*) FROM " . $ecs->table('template') . " WHERE remarks='$remarks' AND theme = '" . $_CFG['formwork'] . "'";
    ...
    $sql = "INSERT INTO " . $ecs->table('template') .
           ...
           " WHERE remarks = '' AND theme = '" . $_CFG['formwork'] . "'".
...

{
...

    {
        ...
        $sql = "DELETE FROM " . $ecs->table('template') . " WHERE remarks='$remarks' AND theme = '" . $_CFG['formwork'] . "'";
...

{
...

    {
        ...
        $sql = "SELECT filename, region, library, sort_order ".
               ...
               " WHERE remarks='$remarks' AND theme = '" . $_CFG['formwork'] . "'".
...

        {
...

            {
                ...
                $lib_content     = file_get_contents(ROOT_PATH . 'template/' . $_CFG['formwork'] . $val['library']);
...

            {
                ...
                $temple_file = ROOT_PATH . 'template/' . $_CFG['formwork'] . '/' . $file . '.dwt';
...

{
    ...
    $lib_file    = '../template/' . $curr_template . '/library/' . $lib_name . '.lbi';
...

{
    ...
    $dir = ROOT_PATH . 'template/' . $tpl_name . '/';
...

{
...

    {
...

        {
            ...
            $tpl_style_list .= '<span style="cursor:pointer;" onMouseOver="javascript:onSOver(\'screenshot\', \'' . $value . '\', this);" onMouseOut="onSOut(\'screenshot\', this, \'' . $style_info['screenshot'] . '\');" onclick="javascript:setupTemplateFG(\'' . $tpl_name . '\', \'' . $value . '\', \'\');" id="templateType_' . $value . '"><img src="../template/' . $tpl_name . '/images/type' . $value . '_';
