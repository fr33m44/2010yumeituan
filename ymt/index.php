...
elseif ($_REQUEST['act'] == 'main')
{
    ...
    {
	$warning[] = $_LANG['remove_installphp'];
    }

    if (file_exists('../sql'))
    {
	$warning[] = $_LANG['remove_sql'];
    }
...
elseif ($_REQUEST['act'] == 'main_api')
{
	...
	/* $shop_url = urlencode($ecs->url());
	  ...
	  $patch_file = file_get_contents(ROOT_PATH.GROUPON_PATH."/patch_num");
...
{
    ecs_header("Location: shop_config.php?act=list_eidt\n");
    exit;
...
elseif ($_REQUEST['act'] == 'third')
{
	...
	//货号
	require_once(ROOT_PATH . GROUPON_PATH . '/includes/lib_goods.php');
