...
function get_comment_list()
{
    ...
    {
	    $sql = "SELECT group_name FROM " .$GLOBALS['ecs']->table('group_activity'). " WHERE group_id='$row[id_value]'";
        $row['title'] = $GLOBALS['db']->getOne($sql);

        /* 标记是否回复过 */
        $sql = "SELECT content FROM " .$GLOBALS['ecs']->table('comment'). " WHERE parent_id = '$row[comment_id]'";
		$row['reply_content'] = $GLOBALS['db']->getOne($sql);
        $row['is_reply'] =  (!empty($row['reply_content'])) ? $GLOBALS['_LANG']['yes_reply'] : $GLOBALS['_LANG']['no_reply'];
