...
if ($_REQUEST['act'] == 'list_edit')
{
    ...
    $smarty->assign('group_list',   get_settings(array(20)));
    ...
    foreach ($_FILES AS $code => $file)
    {
        ...
        if ((isset($file['error']) && $file['error'] == 0) || (!isset($file['error']) && $file['tmp_name'] != 'none'))
        {
            ...
            else
            {
				...
				elseif ($code == 'group_logo')
                {
                    $file_name = str_replace('{$template}', $_CFG['formwork'], $file_var_list[$code]['store_dir']) . 'logo.gif';
                }
                else
                {
...
function get_settings($groups=null, $excludes=null)
{
	...
	$arr = array(
	              'cache_time','timezone','send_mail_on',
	              'time_format','sms_shop_mobile','enable_gzip','enable_order_check'
				); 
	$sql = "SELECT * FROM " . $ecs->table('shop_config') . " WHERE code " . db_create_in($arr);
	$other_list = $db->getALL($sql);
	foreach($other_list as $key => $item)
	{
		$other_list[$key]['parent_id'] = 20;
	}
