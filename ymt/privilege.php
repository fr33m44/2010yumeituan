...
{
    include_once(ROOT_PATH . GROUPON_PATH .  '/languages/priv_action.php');
    ...
    $sql_query = "SELECT action_id, parent_id, action_code,relevance FROM " .$ecs->table('admin_action').
                 " WHERE parent_id = 0 AND action_id NOT IN (1,7)";
    ...
    $no_priv = array(
					 'shop_authorized','webcollect_manage','flash_manage',
					 'cron','sitemap','file_check','file_priv','reg_fields','db_backup','db_renew','role_manage','lang_edit',
					 'attention_list','magazine_list','article_auto'
					 );
    /* 按权限组查询底级的权限名称 */
    $sql = "SELECT action_id, parent_id, action_code,relevance FROM " .$ecs->table('admin_action').
		   ...
		   " AND action_code NOT " . db_create_in($no_priv);
    $privarr = $db->getAll($sql);
	$sql = "SELECT action_id, parent_id, action_code,relevance FROM " .$ecs->table('admin_action').
           " WHERE action_code in ('cat_manage','cat_drop','attr_manage','bonus_manage','comment_priv')";
    $other_priv = $db->getAll($sql);
    foreach ($other_priv as $key => $pric)
	{
		$other_priv[$key]['parent_id'] = 20;
	}
	$privarr = array_merge($privarr, $other_priv);
