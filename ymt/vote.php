...
elseif ($_REQUEST['act'] == 'list_result')
{
    /* 模板赋值 */
    //$smarty->assign('ur_here',      $_LANG['list_vote']);
    //$smarty->assign('action_link',  array('text' => $_LANG['add_vote'], 'href'=>'vote.php?act=add'));
    ...
    /* 显示页面 */
    assign_query_info();
    $smarty->display('vote_list_result.htm');
}
elseif ($_REQUEST['act'] == 'view_result')
{
    $sql="select * from ".$GLOBALS['ecs']->table('vote_result')." where user_id=".$_GET['id'];
    ...
    $smarty->assign('results',$newresults);
    $smarty->assign('full_page',    1);
    assign_query_info();
    $smarty->display('vote_view_result.htm');
}
/*------------------------------------------------------ */
//-- 排序、分页、查询
/*------------------------------------------------------ */
...
elseif ($_REQUEST['act'] == 'option')
{
    ...
    /* 模板赋值 */
    $smarty->assign('ur_here',      '编辑问题选项');
    $smarty->assign('action_link',  array('href'=>'vote.php?act=list', 'text' => '问题列表'));
    $smarty->assign('full_page',    1);
...
elseif ($_REQUEST['act'] == 'new_option')
{
    ...
    $option_type = json_str_iconv(trim($_POST['option_type']));
    ...
    if (!empty($option_name))
    {
        ...
        {
            $sql = 'INSERT INTO ' .$ecs->table('vote_option'). ' (vote_id, option_name, option_type, option_count) '.
                   "VALUES ('$vote_id', '$option_name', '$option_type', 0)";
            $db->query($sql);
...
function get_votelist_result()
{
    $filter   = array();

    /* 记录总数以及页数 */
    $sql = 'SELECT COUNT(*) FROM ' . $GLOBALS['ecs']->table('vote_result')." group by user_id";
    $filter['record_count'] = $GLOBALS['db']->getOne($sql);

    $filter = page_and_size($filter);

    /* 查询数据 */
    $sql  = 'SELECT * FROM ' .$GLOBALS['ecs']->table('vote_result'). ' group by user_id ORDER BY result_id ASC';
    $res  = $GLOBALS['db']->selectLimit($sql, $filter['page_size'], $filter['start']);
