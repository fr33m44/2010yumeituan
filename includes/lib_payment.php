...
function order_paid($log_id, $pay_status = PS_PAYED, $note = '')
{
	...
	if ($pay_log && $pay_log['is_paid'] == 0)
	{
		...
		if ($order['extension_code'] == 'group_buy' && $order['extension_id'] > 0)
		{
		    ...
		    if (!$GLOBALS['_CFG']['group_rebate'] && $order['parent_id'] > 0 && $group_buy['goods_rebate'] > 0)
		    {

			$sql = "SELECT COUNT(*)  FROM " . $GLOBALS['ecs']->table('order_info') . " AS o " .
				" WHERE  o.extension_code = 'group_buy' " .
				" AND o.parent_id='$order[parent_id]' AND o.user_id='$order[user_id]'" .
				" AND o.order_status " . db_create_in(array(OS_CONFIRMED, OS_SPLITED)) .
				" AND o.pay_status " . db_create_in(array(PS_PAYED, PS_PAYING));
			if ($GLOBALS['db']->getOne($sql) == 1)
			{
			    $info = sprintf('邀请返利', $order['order_sn'], $group_buy['goods_rebate'], 0);
			    log_account_change($order['parent_id'], $group_buy['goods_rebate'], 0, 0, 0, $info);
			    set_affiliate_log($order['order_id'], $order['parent_id'], $order['user_id'], $group_buy['goods_rebate']);
			    $sql = "UPDATE " . $GLOBALS['ecs']->table('order_info') .
		    ...
		    if ($group_buy['goods_type'] == 1 && !$GLOBALS['_CFG']['make_group_card'])
		    {
